        ...
        for table_name in table_order:
            with conn.begin():
                ...
                if table_name == 'users':
                    ...
                    for col in columns:
                        if col == 'role':
                            select_columns.append(f"role::{target_schema}.userrole")
                        elif col == 'gender':
                            select_columns.append(f"gender::{target_schema}.gender")
                        else:
                    ...
                    for col in columns:
                        if col == 'status':
                            select_columns.append(f"status::{target_schema}.appointmentstatus")
                        else:
                            select_columns.append(col)
                    select_str = ', '.join(select_columns)
                    ...
                    for col in columns:
                        if col == 'status':
                            select_columns.append(f"status::{target_schema}.appointmentgroupstatus")
                        else:
                            select_columns.append(col)
                    select_str = ', '.join(select_columns)
                    ...
                    for col in columns:
                        if col == 'day_of_week':
                            select_columns.append(f"day_of_week::{target_schema}.dayofweek")
                        else:
                            select_columns.append(col)
                    select_str = ', '.join(select_columns)
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python copy_schema_data.py <source_schema> <target_schema>")
        sys.exit(1)
