...
export const useNavigation = () => {
  ...
  // Memoize tenant info to avoid recalculation
  ...
  const buildRoute = useCallback((path) => {
    ...
    if (useSlugInUrl && currentTenantSlug) {
      return `/${currentTenantSlug}${finalPath}`;
    }
    return finalPath;
...
/**
 * Hook for navigation with automatic route imports
 * Combines navigation utilities with route constants
 */
export const useAppNavigation = () => {
  ...
  const getRoutes = useCallback(async () => {
    const { ROUTES } = await import('../navigation');
    return ROUTES;
  }, []);
